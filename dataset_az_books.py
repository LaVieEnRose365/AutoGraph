...
class MatchDataset:
    ...
    def load_data(self):
        ...
        if self.args.plus_lightgcn_embed:
            self.num_user_fields += 1
            self.num_item_fields += 1

        if self.args.plus_ccgnn_embed:
            self.num_user_fields += 1
            self.num_item_fields += 1

            # Movie Genre
            self.num_genres = len(self.feature_dict["Movie genre"])
            self.genre_table = json.load(open(os.path.join(self.args.data_dir, "movie-genre.json"), "r"))

        if self.args.plus_topoI2I_embed:
            self.num_user_fields += 1
            self.num_item_fields += 1
    ...
    def get_splited_dataset(self, split):
        ...
        elif self.args.plus_topoI2I_embed:
            return I2IDataset(
                split,
                self.args,
                self.user_data[split],
        ...
        elif self.args.plus_ccgnn_embed:
            return CCGNNDataset(
                split,
                self.args,
                self.user_data[split],
...
class LightGCNDataset(BaseDataset):
    def __init__(self, split, args, user_data, pos_item_ids, hist_ids, hist_mask, num_items, num_neg_items, item_to_users, num_users):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            # temp_i_graphs = self.i_graphs.copy()
            # temp_i_graphs[pos_id] = self.remove_u(self.i_graphs[pos_id], uid)
            # i_graph = dgl.batch(temp_i_graphs)
        #     # temp_i_graphs = self.i_graphs.copy()
        #     # temp_i_graphs = self.i_graphs
        #     # temp_i_graphs[pos_id] = self.remove_u(self.i_graphs[pos_id], uid)
            # i_graph = dgl.batch(self.i_graphs)
        #     i_graph = self.i_graphs
    ...
    def construct_u_graph(self, idx):
        ...
        g.nodes["user"].data["id"] = torch.tensor([uid])
        g.nodes["item"].data["id"] = torch.tensor(hist_ids)
    ...
    def construct_i_graph(self, iid):
        ...
        if len(hist_users) == 0:
            ...
            g.nodes["item"].data["id"] = torch.tensor([iid])
            g.nodes["user"].data["id"] = torch.tensor([self.num_users])
            return g
        else:
            edges_dict = {
                ("user", "clicked", "item"): (
            ...
            g = dgl.heterograph(edges_dict)
            g.nodes["item"].data["id"] = torch.tensor([iid])
            g.nodes["user"].data["id"] = torch.tensor(hist_users)
            return g


    def prepare_i_graphs(self):
        if self.split == "train":
            self.i_graphs = [self.construct_i_graph(iid) for iid in trange(self.num_items)]
        # if self.split != "train":
        #     self.i_graphs = dgl.batch(self.i_graphs)
...
class CCGNNDataset(BaseDataset):
    def __init__(self, split, args, user_data, pos_item_ids, hist_ids, hist_mask, num_items, num_neg_items, num_genres, genre_table):
        super().__init__(split, args, user_data, pos_item_ids, hist_ids, hist_mask, num_items, num_neg_items)
        self.num_genres = num_genres
        self.genre_table = genre_table
        self.genre_neighbors = [[] for _ in range(self.num_genres)]

        for iid, genres in enumerate(self.genre_table):
            # if self.args.cold_start and iid in self.cold_items:
            #     continue
            for genre in genres:
                self.genre_neighbors[genre].append(iid)
    ...
    def __getitem__(self, idx):
        ...
        neg_ids = self.gen_neg_items(idx)
        return self.user_data[idx], self.hist_ids[idx], self.hist_mask[idx], \
                pos_id, neg_ids, self.construct_u_graph(idx), self.i_graphs


    def prepare_i_graphs(self):
        np.random.seed(42)
        self.visible_genre_neighbors = [np.random.choice(i, min(len(i), self.args.degree), replace=False) for i in self.genre_neighbors]
        self.i_graphs = self.construct_i_graph()
    ...
    def construct_u_graph(self, idx):
        ...
        # remap genre id        
        genres = [self.genre_table[iid] for iid in hist_ids]
        ...
        src_g2i = [genre_dict[genre] for genre in genres]
        dst_g2i = np.concatenate([[i_dict[iid]]*len(self.genre_table[iid]) for iid in hist_ids])

        # remap item id
        i_neighbors = np.concatenate([self.visible_genre_neighbors[genre] for genre in genre_dict])
        for iid in i_neighbors:
            if iid not in i_dict:
                i_dict[iid] = len(i_dict)
        
        src_i2g = [i_dict[iid] for iid in i_neighbors]
        dst_i2g = np.concatenate([[genre_dict[genre]]*len(self.visible_genre_neighbors[genre]) for genre in genre_dict])

        inverse_genre_dict = {v: k for k, v in genre_dict.items()}
        inverse_i_dict = {v: k for k, v in i_dict.items()}
        ...
        edge_dict = {
            ...
            ("genre", "contains", "item"): (
                torch.tensor(src_g2i),
                torch.tensor(dst_g2i)
            ),

            ("item", "belongs_to", "genre"): (
                torch.tensor(src_i2g),
                torch.tensor(dst_i2g)
            )
        }

        g = dgl.heterograph(edge_dict)
        g.nodes["user"].data["id"] = torch.tensor([uid])
        g.nodes["genre"].data["id"] = torch.tensor([inverse_genre_dict[i] for i in range(len(inverse_genre_dict))])
        g.nodes["item"].data["id"] = torch.tensor([inverse_i_dict[i] for i in range(len(inverse_i_dict))])

        return g

    def construct_i_graph(self):
        src = np.concatenate([[genre_id] * len(neighbors) for genre_id, neighbors in enumerate(self.visible_genre_neighbors)])
        dst = np.concatenate(self.visible_genre_neighbors)

        edge_dict = {
            ("genre", "contains", "item"): (src, dst),
            ("item", "belongs_to", "genre"): (dst, src)
        }
        ...
        g.nodes["genre"].data["id"] = torch.arange(self.num_genres)
        g.nodes["item"].data["id"] = torch.arange(self.num_items)

        return g



class I2IDataset(BaseDataset):
    def __init__(self, split, args, user_data, pos_item_ids, hist_ids, hist_mask, num_items, num_neg_items):
        super().__init__(split, args, user_data, pos_item_ids, hist_ids, hist_mask, num_items, num_neg_items)
        preds = json.load(open("baseline_refine_topo/pred_score_ml25m.json"))
        for i in range(len(preds)):
           if not self.is_decimal(preds[i]):
               preds[i] = 0

        preds = list(map(lambda x: float(x), preds))
        candidates = json.load(open(os.path.join(self.args.data_dir, "baseline_candidate_ids.json")))

        scores = []
        offset = 0
        for ids in candidates:
            scores.append(preds[offset:offset+len(ids)])
            offset += len(ids)
        
        self.i_neighbors = []
        cold_items = self.cold_items if self.args.cold_start else []
        for score, ids in zip(scores, candidates):
            sorted_idx = np.argsort(score)[::-1]
            sorted_ids = [ids[i] for i in sorted_idx if i not in cold_items]
            self.i_neighbors.append(sorted_ids[:self.args.degree])

        self.construct_i_graph()
        logger.info(f"Item graphs prepared.")
    ...
    def construct_u_graph(self, idx):
        ...
        for hist_id in hist_ids:
            for iid in self.i_neighbors[hist_id]:
                if iid not in i_dict:
                    i_dict[iid] = len(i_dict)
        
        reverse_i_dict = {v: k for k, v in i_dict.items()}
        src_i2i = [i_dict[iid] for hist_id in hist_ids for iid in self.i_neighbors[hist_id]]
        dst_i2i = np.concatenate([[i]*len(self.i_neighbors[hist_id]) for i, hist_id in enumerate(hist_ids)]).tolist()
        ...
        edges_dict = {
            ...
            ("item", "similar_to", "item"): (
                torch.tensor(src_i2i + dst_i2i),
                torch.tensor(dst_i2i + src_i2i)
            )
        }

        g = dgl.heterograph(edges_dict)
        g.nodes["user"].data["id"] = torch.tensor([uid])
        g.nodes["item"].data["id"] = torch.tensor([reverse_i_dict[i] for i in range(len(reverse_i_dict))])

        return g


    def construct_i_graph(self):
        src = np.concatenate([[i] * len(neighbors) for i, neighbors in enumerate(self.i_neighbors)])
        dst = np.concatenate(self.i_neighbors)
        self.i_graphs = dgl.graph((src, dst), num_nodes=self.num_items)
...
def data_collator_ccgnn(features):
    batch = {
        ...
        "u_graphs": dgl.batch([f[5] for f in features]),
        "i_graphs": features[0][6]
    }
